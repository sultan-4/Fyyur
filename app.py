...
def venues():
  ...
  data=[]
  place= Venue.query.distinct(Venue.city,Venue.state)
  ...
  for x in place:
    ...
    for y in venues:
      diffrent_place['venues'].append({
        "id":y.id,
        "name":y.name
      })
  
    data.append(diffrent_place)
...
def search_venues():
  ...
  response={
    "count":count,
    "data":[]
    # "num_upcoming_shows": Venue.query.filter_by(id=venues.id).join(Shows,Shows.venue_id == Venue.id).filter(Shows.time > currentTime).count()
  ...
  for venue in venues:
    response['data'].append({
      'id':venue.id,
      'name':venue.name,
      "num_upcoming_shows": Venue.query.filter_by(id=venue.id).join(Shows,Shows.venue_id == Venue.id).filter(Shows.time > currentTime).count()
      })
...
def create_venue_submission():
  ...
  try:
    ...
    venue = Venue(name=name , genres=genres,city=city,state=state,address=address
                ,phone=phone,website=website,image_link=image,facebook_link=facebook,
...
def delete_venue(venue_id):
  ...
  try:
    #delete all venues and shows assigend to it
...
def search_artists():
  ...
  for artist in artists:
    response['data'].append({
      'id':artist.id,
      'name':artist.name,
      "num_upcoming_shows": Artist.query.filter_by(id=artist.id).join(Shows,Shows.artist_id == Artist.id).filter(Shows.time > currentTime).count()
      })
...
def show_artist(artist_id):
  ...
  currentTime =  datetime.now().strftime('%Y-%m-%d %H:%M:%S')
  shows =Shows.query.filter_by(artist_id=artist.id)
  for show in shows:
   print('first',show)
...
def edit_artist_submission(artist_id):
  ...
  try:
    ...
    artist.seeking_venue= True if request.form.get('seeking_talent') == 'y' else False
...
def create_artist_submission():
  try:
    ...
    seeking_description = request.form['seeking_description']
    artist = Artist(name=name,genres=genres,city=city,state=state,phone=phone,website=website,image_link=image,facebook_link=facebook,seeking_venue=seeking_venue,seeking_description=seeking_description)
...
def create_show_submission():
  ...
  try:
   ...
   show = Shows(artist_id=artist_id,venue_id=venue_id,time=time)
   db.session.add(show)
   db.session.commit()
